...
tidy_up_sentence<-function(srcLine) {
		...
		srcLine<-gsub("[%#&ÖÓÉÊÓÍ·ìí…ÌŽÆàÅè¡›áÏÚÛ‡†ºÎâæ¥Šæžä½¿ã£ã¦å°ã€Œ¨€½œ£¿¾Ÿï¼šï¼‰™ïœÃ©¢¶ƒ§Â¤“³»®Ä¹³»®««çª‹ˆ˜Ñ•ÐµÙØ±¬Ë„²ë¸êéî¯––×Ç\002\020\002#\023ð\035\037\177\032\031\003]","",srcLine)
		return(srcLine)
}
...
##		options <- stringi::stri_opts_brkiter(type="word", skip_word_none = TRUE)
##		ngram_tbl<-cbind(data.table(word=srcWords,count=1),data.table(t(unlist(stringi::stri_split_boundaries(srcWords)))))
##		ngrams<-ngram_tbl[, .(count=sum(count)), by=word]
##		return(ngrams)
##	}
