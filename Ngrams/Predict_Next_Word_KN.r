...
predict_next_word<-function(input_str) {
	suppressPackageStartupMessages(library(data.table))
		...
		input_wrds<- unlist(strsplit(input_str, "\\s+"), use.names=FALSE)
			...
			if (len >= 3) {
				wrds_matched <- quadgrams[J(input_wrds[len-2], input_wrds[len-1], input_wrds[len]),.(prob=pKN*0.04,word=word4,ngram="fourgram")]
			...
			if (len >= 2 & nrow(predicted_words) < 4) {
				wrds_matched <- trigrams[J(input_wrds[len-1], input_wrds[len]),.(prob=pKN*0.02,word=word3,ngram="trigram")]
			...
			if (len >= 1 & nrow(predicted_words) < 4) {
				wrds_matched <- bigrams[J(input_wrds[len]),.(prob=pKN*0.001,word=word2,ngram="bigram")]
				predicted_words <- rbind(predicted_words, wrds_matched)
				predicted_words <-predicted_words[!is.na(predicted_words$word)]
			...
			if (nrow(predicted_words) < 4) {
				...
				if (len!=0) {
					...
					if (length(wrds_matched)!=0) {
						wrds_matched<-unigrams[ unigrams$word1 %in% wrds_matched,.(prob=pKN,word=word1,ngram="unigram")]
					} 
					if (length(wrds_matched)!=0)
					predicted_words <- rbind(predicted_words, wrds_matched)
				}
				if (nrow(predicted_words)==0) {
				    predicted_words<-unigrams[ unigrams$word1 %in% c("the","and"),.(prob=pKN,word=word1,ngram="unigram")]
				}
