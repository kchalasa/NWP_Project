...
## Description: This Script defines the Next Word Prediction Function
## 1. Input Parameter: Input Phrase entered by the end-user using SHINY application
## 2. Output: Returns a data.table 
##           a. Word(s) - Top four word predictions
##           b. Conditional Probability 
##           c. ngram used in prediction
##########################################################
...
predict_next_word<-function(input_str) {
	suppressPackageStartupMessages(library(data.table))
		...
		input_wrds<- unlist(strsplit(input_str, "\\s+"), use.names=FALSE)
			...
			predicted_words<-head(predicted_words,4) 
	    setorder(predicted_words,-prob)
